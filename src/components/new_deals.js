...
function NewProductCard(props) {
    ...
    switch(props.type) {
        case 'first':
            klass = 'new-deals__product_first'
            break;
        case 'last':
            klass = 'new-deals__product_last'
            break;
        case 'active':
            klass = 'new-deals__product_active'
            break;
    }

    return (
        <NavLink to={'/productcard/' + props.id}>
            <div className={'new-deals__product ' + klass} style={{backgroundImage: backgroundImageValue, backgroundSize: 'contain' }}>
                
                {props.type === 'active' && 
                    <div className={props.isFavorite ? 'new-deals__product_favorite_chosen' : 'new-deals__product_favorite'} 
                    onClick={e => { e.preventDefault(); e.stopPropagation(); props.onFavorite(props.id);}}></div>}
            </div>        
        </NavLink>
    )
}

function NewProductsMenu(props) {
